<template>
  <div class="shop-header">
    <nav class="shop-nav" :style="{backgroundImage:`${info.bgImg}`}">
      <a class="back" @click="shopHeadeBack"> 
        <i class="iconfont icon-zuojiantou"></i> 
      </a>
    </nav>
    <div class="shop-content" @click="headerClose=true">
      <img
        class="content-image"
        :src="`${info.avatar}`"
      />
      <div class="header-content">
        ...
        <div class="shop-message">
          ...
          <span class="shop-message-detail">
            {{info.description}} <span>约 {{info.ratingCount}} 分钟</span>
          </span>
          <span class="shop-message-detail">距离 {{info.distance}}</span>
        </div>
      </div>
    </div>
    <div class="shop-header-discounts" @click="headerDiscount=true">
      <div class="discounts-left">
        <div class="activity activity-green">
          <span class="content-tag"> <span class="mini-tag">首单</span> </span>
          <span class="activity-content ellipsis">新用户下单立减17元(不与其它活动同享)</span>
        </div>
      </div>
      <div class="discounts-right">4 个优惠</div>
    </div>
    <div class="shop-brief-modal" v-show="headerClose">
      <div class="brief-modal-content">
        ...
        <div class="mask-footer" >
          <span 
            class="iconfont icon-iconclose" 
            @click="headerClose=false"
          >
          </span>
        </div>
      </div>
    ...
    <div class="activity-sheet" v-show="headerDiscount">
      <div class="activity-sheet-content">
        <h2 class="activity-sheet-title">优惠活动</h2>
        <ul class="list">
          <li 
            class="activity-item activity-green"
            v-for="(sup,index) in info.supports"
            :key="index"
            :class="headerDiscountColor[sup.type]"
          >
            <span class="content-tag">
        ...
        <div class="activity-sheet-close">
          <span class="iconfont icon-iconclose" @click="headerDiscount=false"></span>
        </div>
      </div>
...
export default {
  ...
  data(){
    return {
      headerClose:false,//店铺介绍开关阀
      headerDiscount:false,//优惠活动开关阀
      headerDiscountColor:['activity-green','activity-red','activity-orange'],
    }
  },
  computed:{
    ...mapState(['info']),
